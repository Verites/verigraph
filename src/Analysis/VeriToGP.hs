...
nodesToGP _  _    a []              = a
nodesToGP tg side a (NodeId b:xs) = nodesToGP tg side (GP.Node n b side : a) xs
   where
     (NodeId n) = fromJust $ GM.applyNode tg (NodeId b)

edgesToGP :: GM.TypedGraph a b -> String -> Graph a b -> [GP.Edge] -> [EdgeId] -> [GP.Edge]
edgesToGP _  _    _ a []              = a
edgesToGP tg side g a (EdgeId b:xs) = edgesToGP tg side g (GP.Edge typ b src tgt side : a) xs
   where
      (EdgeId typ) = fromJust $ GM.applyEdge tg (EdgeId b)
      src = GP.Node n1 (read (show $ fromJust $ sourceOf g (EdgeId b)) :: Int) side
      tgt = GP.Node n2 (read (show $ fromJust $ targetOf g (EdgeId b)) :: Int) side
