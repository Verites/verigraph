...
data TypedGraphMorphism a b = TypedGraphMorphism {
    domainGraph   :: TypedGraph (Maybe a) (Maybe b)
  , codomainGraph :: TypedGraph (Maybe a) (Maybe b)
...
buildTypedGraphMorphism :: TypedGraph (Maybe a) (Maybe b) -> TypedGraph (Maybe a) (Maybe b)
                        -> GraphMorphism (Maybe a) (Maybe b) -> TypedGraphMorphism a b
...
instance Valid (TypedGraphMorphism a b) where
    validate (TypedGraphMorphism dom cod m) =
      mconcat
        ...
        , ensure (d == GM.compose c m) "Morphism doesn't preserve typing"
        ]
      where
        d = toGraphMorphism dom
        c = toGraphMorphism cod
...
createEdgeOnDomain e1 s1 t1 tp e2 tgm =
  tgm { domainGraph = createEdgeOnBaseGraph e1 s1 t1 tp (domainGraph tgm)
...
createEdgeOnCodomain e2 s2 t2 tp tgm =
  tgm { codomainGraph = createEdgeOnBaseGraph e2 s2 t2 tp (codomainGraph tgm)
...
createNodeOnDomain n1 tp n2 tgm =
  tgm { domainGraph = createNodeOnBaseGraph n1 tp (domainGraph tgm)
...
createNodeOnCodomain n2 tp tgm =
  tgm { codomainGraph = createNodeOnBaseGraph n2 tp (codomainGraph tgm)
...
updateNodeRelation n1 n2 tp tgm =
  TypedGraphMorphism { domainGraph = updateNodeTyping n1 tp (domainGraph tgm)
                     , codomainGraph = updateNodeTyping n2 tp (codomainGraph tgm)
...
removeNodeFromDomain n tgm =
  tgm { domainGraph = removeNodeFromBaseGraph n (domainGraph tgm)
      , mapping = GM.removeNodeFromDomain n (mapping tgm)
      }

-- | Removes an edge from the domain of a typed graph morphism
removeEdgeFromDomain :: EdgeId -> TypedGraphMorphism a b -> TypedGraphMorphism a b
removeEdgeFromDomain e tgm =
  tgm { domainGraph = removeEdgeFromBaseGraph e (domainGraph tgm)
...
removeNodeFromCodomain n tgm =
  tgm { codomainGraph = removeNodeFromBaseGraph n (codomainGraph tgm)
...
removeEdgeFromCodomain e tgm =
  tgm { codomainGraph = removeEdgeFromBaseGraph e (codomainGraph tgm)
...
idMap :: TypedGraph (Maybe a) (Maybe b) -> TypedGraph (Maybe a) (Maybe b) -> TypedGraphMorphism a b
idMap gm1 gm2 =
...
reflectIdsFromTypeGraph tgm = buildTypedGraphMorphism newDomain newCodomain newMaps
  where
    gmDomain = domainGraph tgm
    gmCodomain = codomainGraph tgm

    newNodes gm = map (GM.applyNodeIdUnsafe (toGraphMorphism gm)) (nodeIds (untypedGraph gm))
    newEdges gm = map (\x -> (GM.applyEdgeIdUnsafe (toGraphMorphism gm) (edgeId x),
                              GM.applyNodeIdUnsafe (toGraphMorphism gm) (sourceId x),
                              GM.applyNodeIdUnsafe (toGraphMorphism gm) (targetId x)))
                      (edges $ untypedGraph gm)

    newDomain = fromGraphMorphism (foldr (\(e,s,t) -> GM.createEdgeOnDomain e s t e)
                                  (foldr (\x -> GM.createNodeOnDomain x x)
                                      (GM.empty empty (untypedGraph gmDomain))
                                      (newNodes gmDomain))
                                      (newEdges gmDomain))
    newCodomain = fromGraphMorphism (foldr (\(e,s,t) -> GM.createEdgeOnDomain e s t e)
                                  (foldr (\x -> GM.createNodeOnDomain x x)
                                      (GM.empty empty (untypedGraph gmCodomain)) (newNodes gmCodomain)) (newEdges gmCodomain))
...
reflectIdsFromCodomain tgm =
  let
    ...
    typedB' = fromGraphMorphism (GM.empty empty typeGraph)
    ...
    addNodes = foldr (\n -> createNodeOnDomain (applyNodeIdUnsafe tgm n) (GM.applyNodeIdUnsafe (toGraphMorphism typedA) n) (applyNodeIdUnsafe tgm n)) initial nodes
    addEdges = foldr (\e ->
      createEdgeOnDomain (applyEdgeIdUnsafe tgm (edgeId e))
                         (applyNodeIdUnsafe tgm (sourceId e))
                         (applyNodeIdUnsafe tgm (targetId e))
                         (GM.applyEdgeIdUnsafe (toGraphMorphism typedA) (edgeId e))
...
reflectIdsFromDomains (m,e) =
  let
    ...
    typedG' = fromGraphMorphism (foldr (\(e,s,ta,ty) -> GM.createEdgeOnDomain e s ta ty)
                                (foldr (uncurry GM.createNodeOnDomain) (GM.empty empty tg) newNodes)
                                        newEdges)
