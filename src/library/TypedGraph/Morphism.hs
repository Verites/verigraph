...
import           TypedGraph.Partitions.GPToVeri  (mountTypedGraphMorphisms)
...
instance EpiPairs (TypedGraphMorphism a b) where
  ...
  createJointlyEpimorphicPairs inj m1 m2 = map (mountTypedGraphMorphisms m1 m2) (genGraphEqClass (mixGM (m1,inj) (m2,inj)))

  createAllSubobjects inj m1 = map fst part
    where
      m2 = buildGraphMorphism G.empty G.empty [] []
      part = map (mountTypedGraphMorphisms m1 m2) (genGraphEqClass (mixGM (m1,inj) (m2,inj)))

  createJointlyEpimorphicPairsFromNAC config r nac =
    map (mountTypedGraphMorphisms r (codomain nac)) (genGraphEqClass (mixNac (r, matchInj) (nac, nacInj)))
  ...
  calculateCommutativeSquaresAlongMonomorphism (m1,inj1) (m2,inj2) = filt
    where
      cod1 = codomain m1
      cod2 = codomain m2
      allPairs = map (mountTypedGraphMorphisms cod1 cod2) (genGraphEqClass (mixGM (cod1,inj1) (cod2,inj2)))
