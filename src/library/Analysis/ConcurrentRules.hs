...
allConcurrentRules :: (DPO m, EpiPairs m, Eq (Obj m)) => CRDependencies -> NacSatisfaction -> MatchRestriction -> [Production m] -> [Production m]
...
maxConcurrentRule :: (DPO m, EpiPairs m, Eq (Obj m)) => CRDependencies -> NacSatisfaction -> MatchRestriction -> [Production m] -> Production m
maxConcurrentRule dep nacInj injectiveOnly rules = last $ maxConcurrentRules dep nacInj injectiveOnly rules

maxConcurrentRules :: (DPO m, EpiPairs m, Eq (Obj m)) => CRDependencies -> NacSatisfaction -> MatchRestriction -> [Production m] -> [Production m]
...
concurrentRules :: (DPO m, EpiPairs m, Eq (Obj m)) => CRDependencies -> NacSatisfaction -> MatchRestriction -> Production m -> Production m -> [Production m]
...
maxConcurrentRuleForLastPair :: (DPO m, EpiPairs m, Eq (Obj m)) => CRDependencies -> NacSatisfaction -> MatchRestriction -> Production m -> Production m -> Production m
...
epiPairsForConcurrentRule :: (DPO m, EpiPairs m)
                             => CRDependencies -> NacSatisfaction -> MatchRestriction
...
epiPairsForConcurrentRule _ _ matchInj c n =
  let allPairs = createPairs (matchInj == MonoMatches) (codomain (right c)) (codomain (left n))
      isValidPair (lp, rp) = satsGluing matchInj (right c) lp && satsGluing matchInj (left n) rp
  in filter isValidPair allPairs

concurrentRuleForPair :: (DPO m, EpiPairs m, Eq (Obj m)) => NacSatisfaction -> MatchRestriction -> Production m -> Production m -> (m, m) -> Production m
