module Abstract.Category.AdhesiveHLR.Core (
  AdhesiveHLR(..)
)
...
class (Cocomplete morph) => AdhesiveHLR morph where
  ...
  {- | Given two objects @A@ and @B@ of the M-Adhesive Category, finds all the morphisms
      @m : A -> B@ such that @m@ is in the @M@ class of morphisms -}
  findMMorphisms :: Obj morph -> Obj morph -> [morph]

  {- | Given a morphism @m@ it returns @True@ if @m@ belongs to the @M@ class of morphisms and
        @False@ otherwise-}
  isMMorphism :: morph -> Bool
